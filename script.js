...
if (modal) {
    ...
    modal.addEventListener('click', function(e) {
        if (e.target === modal || e.target.classList.contains('modal-close')) {
            ...
            }, 300); // Keep the timeout for potential CSS transitions
        }
    });
...
function updateBusinessStatus() {
    ...
    // Ensure correct timezone for business hours. You might need to adjust this based on your actual business location.
    const currentDay = now.toLocaleDateString('en-US', { weekday: 'long', timeZone: 'America/New_York' }).toLowerCase();
    ...
    // Update current time display (using local timezone for display)
    const timeDisplay = document.getElementById('currentTime');
    if (timeDisplay) {
        timeDisplay.textContent = new Date().toLocaleString(); // Display in user's local time
    }
}
...
function initializeBookingSystem() {
    ...
    if (bookingForm) {
        bookingForm.addEventListener('submit', function(e) {
            e.preventDefault();
            // Improved feedback for booking submission
            const formData = new FormData(this);
            const name = formData.get('name'); // Assuming your form has an input with name="name"
            alert(`Booking submitted successfully for ${name || 'Guest'}! We will contact you shortly.`);
            bookingModal.style.display = 'none';
            document.body.classList.remove('modal-open');
            // In a real application, you would send this data to a server here
            console.log('Booking data submitted:', Object.fromEntries(formData));
        });
    }
}
...
function populateTimeSlots() {
    ...
    dateInput.addEventListener('change', function() {
        ...
        if (!hours.closed) {
            ...
            timeSelect.innerHTML += '<option value="" disabled>Closed on this day</option>';
        }
    });
}
